...
# this package provides methods to blast against
# complete genomes
...
sub run_job {
	...
	# we'll against the full database file
	# genomes databases are rather small 
	# compared to EMBL or SpTrEBML
...
sub command_line {
	...
	return $tool->executable_name." $dbfile $queryfile";
    }

    my $dbname = $tool->dbname();
    return $tool->executable_name." $dbname $queryfile";
}
...
sub dbsequence {
    ...
    # init fasta storage for this tool if not done yet
    if (!GENDB::Tools::FastaStorage->has_key($used_tool->name)) {
	# put database into storage
      GENDB::Tools::FastaStorage->put_into_storage ($used_tool->name,
						    $used_tool->dbname);
    }
    # retrieve the fasta entry
    return GENDB::Tools::FastaStorage->get_fasta($used_tool->name,
						 $fact->dbref." ".$fact->description);
}
