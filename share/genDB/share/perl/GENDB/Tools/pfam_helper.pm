...
#	open (PFAMRUN, "$cmdline |");  
    open(PFAMRUN, '-|',$cmdline) or die "can't open PFAMRUN: $!";
    my @output = <PFAMRUN>;
    open(OUT, '>', $outputfile) or die "can't create $outputfile: $!";
    print OUT @output;
    close(OUT) or warn "can't close $outputfile: $!";
    close(PFAMRUN) or warn "can't close PFAMRUN: $!";

   # my $hmm_parser=Bio::Tools::HMMER::Results->new(-fh => *PFAMRUN,
   # 					       -type => 'hmmpfam');
    my $hmm_parser = Bio::SearchIO->new(
                                        -format =>  'hmmer3',
                                        -file   =>  $outputfile,
#                                        -fh     =>  *PFAMRUN,
                     );
    ...
    while (my $hit = $hmm_rslt->next_model) {
        ...
        $new_fact->dbref($hit->name());
        $new_fact->description($pfam->pfam_description($hit->name()));
        while (my $hsp = $hit->next_hsp) {
    	    $new_fact->orffrom($hsp->start('hit'));
         	$new_fact->orfto($hsp->end('hit'));
         	$new_fact->dbfrom($hsp->start('query'));
         	$new_fact->dbto($hsp->end('query'));
         	$new_fact->toolresult($hsp->significance());
         	$new_fact->information($hsp->score());
         	$new_fact->tool_id($tool->id);
        }
    }
#	my $pfam = CGRB::PFAM->new();
...
#  # 	$new_fact->description($domain->hmmacc);
   # 	$new_fact->description($pfam->pfam_description($domain->hmmname()));
