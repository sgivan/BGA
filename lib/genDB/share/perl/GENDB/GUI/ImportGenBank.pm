package GENDB::GUI::ImportGenBank;

# this packages provides a dialog to import GenBank formatted
# data into GENDB
...
use GENDB::Tools::Importer::GenBank;

### 
# config area

my $config_dir_parameter_name='GenBank_dialog_dir';
...
# dialog to add an genbank file content
sub add_genbank {
    my ($call, $mainref)=@_;
   
    my $dialog = new Gtk::Dialog;
    $dialog->title("Import from GenBank file:");
    $dialog->set_modal(1);
	...
	if ($import_file ne "") {
	    # create an importer
	    my $importer = GENDB::Tools::Importer::GenBank->new($import_file);
...
sub non_unique_name_dialog {
    ...
    my $label = new Gtk::Label("GENDB requires all contig names to be unique.\nThe GenBank file contains an entry named '$contig_name', which is already known to GENDB.");
